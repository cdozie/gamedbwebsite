...
import { limittyping, limittypingmax, isNumbCheck, listfetch, isBetween, displayerror, tableclassnames, listfetchdictlist  } from './mylistfunctions';
...
// Add Check Box for Filtering Out Unrated Ones
  ...
  const onStatusClick = (e) =>{
    ...
    const onGPRClick = (e)  =>{
      setGNamePR(() => e.target.id)
      setGNameHP(() =>"")
      setPRVal(() => e.target.textContent)

  
    }
    // const send_table_change = (e, dataname) => {
    //   $.ajax({
    //     type:'POST',
    //     url:'/mylistfeeder',
    //     data:{
    ...
    // function EditHoursPlayed (props){
    //   // console.log(gname)
    //   // seteditHPVal(()=> props.GHP)
    //   if ((gnamehp == `${props.Name}`) ){
    //     return(
    //       <td id = "editHP">
    //       <Form >
    //         <Form.Control type = "number" min = "0"  value = {HPVal} placeholder="Hours Played"  name = {props.Name} className='w-auto mx-auto' onChange = {e => {limittyping(e, e.target.value <0, 0); onHoursChange(e)}}/>
    ...
    //     if (HPVal !== ""){
    //       return(
    //       <td value = {HPVal}  id = {`${props.Name}`} onClick = {onHPClick} className ="table-hours-played">{props.GHP}</td>
    //       )
    //     }
    //     else {
    //       return(
    //       <td value = {props.GHP}  id = {`${props.Name}`} onClick = {onHPClick} className ="table-hours-played">{props.GHP}</td>
    //       )
    //     }
    //   } 
    // }
    ...
    function EditStatus(props){
      ...
      <select value = {STVal} name=  {`${props.Name}`} autoComplete="off"  className = "form-select form-control mx-auto w-100 game-form game-highlight "  placeholder = "Choose Status:" required onChange ={onSTChange}> ' + 
            ...
            <div className='my-list-table my-list'>
                <Table  bordered>
                    ...
                    <tr key = {`${game.Name}`}>
                        ...
                        <td value = {game.Name}><a href ={`/game/${game.Name}`}>{game.Name}</a></td>
