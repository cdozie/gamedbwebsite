...
var lists = document.querySelectorAll('.hs');
...
function searchGame() {
  var gamesearch = $("#gameSearch, #gameSearch3").val(); //console.log(gameSearch)

  var api_key = "bbd0f6e116b145bd81a72ee35824f71f";
  var url2 = "https://api.rawg.io/api/games?key=".concat(api_key, "&page_size=10&search=").concat(gamesearch);
  axios.get(url2).then(function (response) {
    var gameresponse = response.data; //console.log (gameresponse.results.length)

    slugs = [];
    names = [];
    images = [];

    for (var i = 0; i < gameresponse.results.length; i++) {
      ...
      var bgimage = gameresponse.results[i].background_image; //console.log(bgimage)

      var dropdownopts = document.getElementById("game".concat(i + 1));
      var dropdownimages = document.getElementById("game".concat(i + 1, "images"));
      dropdownopts.innerText = "".concat(gamename);
      dropdownimages.src = "".concat(bgimage);
    } //console.log (names)
...
$('.gamedboptions').on('click', function () {
  ...
  submissionindex = parseInt(last) - 1; // addsubmission = $('#gameSearch').val()
  // localStorage.setItem("addsubmission", addsubmission);

  localStorage.setItem("submissionindex", submissionindex);
  submissionslug = slugs[submissionindex]; // console.log(alert(slugs))

  localStorage.setItem("submissionslug", submissionslug); // console.log(alert(localStorage.getItem("submissionslug")))
...
$(".rand-name-forms").on('click', function () {
  ...
  $("#rand-game-list-form".concat(randgameidnumb)).trigger("submit");
});
...
$(function () {
  ...
  $('.rankings,.rankings2,.account-mcr,.tablerating,.accountranks, .vargamerankings, .varpersonalrankings').each(function (index) {
    ...
    $.each(mc, function (name, value) {
      ...
      second = parseInt(name.split('-')[1], 10); // console.log(between(dc, first, second));

      if (between(dc, first, second)) {
        th.addClass(value); // console.log(value)
...
$(function () {
  ...
  $.each(mc, function (name, descriptext) {
    ...
    var second = parseInt(name.split('-')[1], 10); // console.log(between(dc, first, second));

    if (between(dattr, first, second)) {
      description.text(descriptext); // console.log(value)
      // $('.rating-border').css({'border-color': name})
    } // else{
...
var classundefined = 1; // var firstclass
// var editbutton
// var editform
  ...
  if ($(".var-page-container").attr('id') == "yes") {
    $('.var-personal-rating-data, .var-status-data, .var-hours-played-data').on('click', function () {
      ...
      if (modiefiedlements.length == 0 && firstclass == "var-personal-rating-data") {
        ...
        th.html('<select name="status" id = "var-edit-status" autocomplete="off"  class = "form-select form-control mx-auto w-100 game-form game-highlight "  placeholder = "Choose Status:" required> ' + '<option disable selected value>Choose Status:</option>' + '<option>Plan to Play</option>' + '<option>Dropped</option>' + '<option>Playing</option>' + '<option>On Hold</option>' + '<option>Completed</option>' + '<option>Wishlist</option>' + '</select>'); // Removing Classes With Background Overlays For Colors
...
$(function () {
  ...
  $('.table-personal-rating, .table-hours-played, .table-status').on('click', function () {
    ...
    if (modifyingelements.length == 0 && $(this).attr('class') == "table-status") {
      $(this).html('<select name="status" id = "edit-select-status" autocomplete="off"  class = "form-select form-control mx-auto w-100 game-form game-highlight "  placeholder = "Choose Status:" required> ' + '<option disable selected value>Choose Status:</option>' + '<option>Plan to Play</option>' + '<option>Dropped</option>' + '<option>Playing</option>' + '<option>On Hold</option>' + '<option>Completed</option>' + '<option>Wishlist</option>' + '</select>');
      $(this).addClass('modifying');
      globalThis.classname = $(this).attr('class'); //var conceptName = $('#aioConceptName').find(":selected").text();
...
$("#add-form").on('submit', function () {
  ...
  $("#gameSearch2, #gameSearch, #gameSearch3").html("".concat(slugs[gameindex]));
...
$("#add-form").on("submit", function () {// localStorage.clear()
